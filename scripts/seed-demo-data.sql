...
-- Every row carries user_id = 1 (the seeded owner): user_id leads each natural
-- key, so the ON CONFLICT targets below must include it.
...
SELECT 1, 'google_health', 'demo-sleep-'||d,
       'fitbit_air', 'device',
...
ON CONFLICT (user_id, source, session_uid) DO NOTHING;
...
SELECT 1, 'google_health', 'demo-sleep-'||d, p.stage,
...
ON CONFLICT (user_id, source, session_uid, started_at) DO NOTHING;

-- Daily scalar metrics (HRV, RHR, respiratory rate, SpO2 avg/min).
INSERT INTO raw.daily_metrics (user_id, source, device, recording_method, metric, value, unit, local_date, tz_offset_minutes, natural_key, raw)
SELECT 1, 'google_health','fitbit_air','device', m.metric, m.value, m.unit, d, -240, m.metric||'|'||d, '{}'
FROM generate_series(0,44) g,
     LATERAL (SELECT (DATE '2026-07-14' - g) AS d) x,
     LATERAL (VALUES
        ...
        ('skin_temp_nightly',  (33.4 + ((g*3) % 9) * 0.1)::float, 'C'),
        ('skin_temp_baseline', 33.8::float,                       'C')
     ) AS m(metric,value,unit)
ON CONFLICT (user_id, source, metric, local_date) DO NOTHING;

-- Daily step totals, as continuous samples (one/day).
INSERT INTO raw.samples (user_id, source, device, recording_method, metric, value, unit, valid_from, valid_to, natural_key, raw)
SELECT 1, 'google_health','fitbit_air','device', s.metric, s.value, s.unit,
...
ON CONFLICT (user_id, source, metric, natural_key, valid_from) DO NOTHING;
