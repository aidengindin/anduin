...
def metric_series(conn: Connection, metric: str, start: date | datetime, end: date | datetime) -> dict[str, Any]:
    ...
    # Counter metrics are daily rollups, so their buckets are pinned to a day:
    # zero-filling an hourly bucket would emit 23 phantom zeros per real day.
    bucket = "1 day" if m.get("zero_fill") else _bucket_for_range(start_dt, end_dt)
    dcol = m["date_col"]
    agg = f"""
